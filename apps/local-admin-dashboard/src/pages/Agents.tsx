          ...
          <tbody className="divide-y divide-border">
            {loading ? (
              ...
              <tr key={agent.agent_id} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="px-6 py-4 text-muted-foreground font-mono text-xs">{agent.agent_id}</td>
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-medium ${
                    ...
                    <span className={`h-1.5 w-1.5 rounded-full ${agent.meta.status === 'active' ? 'bg-emerald-500' : 'bg-muted-foreground'}`} />
                    {agent.meta.status}
                  </span>
                </td>
                <td className="px-6 py-4 text-muted-foreground">{agent.runtime.version || 'Unknown'}</td>
                <td className="px-6 py-4 text-muted-foreground">{new Date(agent.meta.updated_at).toLocaleString()}</td>
