...
fn test_unauthorized_tenant() {
    ...
    assert_eq!(result.unwrap_err().to_string(), "Unauthorized map update: tenant_id mismatch");
}
...
fn test_anti_rollback_generation() {
    ...
    let result = updater.apply_update(update);
    assert!(result.is_err());
    assert_eq!(result.unwrap_err().to_string(), "Unauthorized map update: generation rollback attempt");
}
...
fn test_high_risk_map_requires_signature() {
    ...
    assert!(result.is_err());
    assert_eq!(result.unwrap_err().to_string(), "Unauthorized map update: signature strictly required for this source/map");
