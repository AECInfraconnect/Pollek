...
#[derive(Serialize)]
struct DeploySessionResponse {
    id: String,
    feasibility: PolicyFeasibilityResult,
    status: DeploymentSessionStatus,
}

fn agent_id_from_scope(scope: &DeploymentScope) -> Option<String> {
    match scope {
...
async fn rollback_deploy_session(
    Path((_tenant, id)): Path<(String, String)>,
    State(state): State<AppState>,
) -> ApiResult<(StatusCode, Json<DeployReport>)> {
    let mut session = state
