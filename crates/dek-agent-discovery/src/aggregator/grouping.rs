//! Candidate identity + surface grouping: the dedup/coalesce engine that
//! buckets discovery evidence by stable identity, merges duplicate
//! candidates, and groups child surfaces under their parent agent.

use super::*;

pub(super) fn service_slug(value: &str) -> String {
    let slug = value
        .chars()
        .map(|ch| {
...
pub(super) fn default_canonical_service_id(
...
pub(super) fn merge_candidate_semantics(
...
pub(super) fn coalesce_by_identity(
    tenant: &str,
...
pub(super) fn port_from_url(url: &str) -> Option<u16> {
...
pub(super) fn canonical_signature_alias(id: &str) -> &str {
    match id {
...
pub(super) fn candidate_identity_bucket(candidate: &DiscoveredAgentCandidateV2) -> String {
    if matches!(
...
pub(super) fn apply_surface_grouping(
...
pub(super) fn is_better_name(new: &str, old: &str) -> bool {
    let bad = |s: &str| {
        s == "Unknown Agent"
...
pub(super) fn aggregate_by_merge_key(
    tenant_id: &str,
    device_id: &str,
