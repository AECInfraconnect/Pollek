...
pub fn emit_span(event: &AgentObservationEvent) {
    ...
    let mut attrs = vec![
        ...
        KeyValue::new("pollek.tenant_id", event.tenant_id.clone()),
    ];
...
pub fn emit_usage_span(event: &AiUsageEventV1) {
    ...
    let mut attrs = vec![
        ...
        KeyValue::new("pollek.agent_type", format!("{:?}", event.agent_type)),
        KeyValue::new("pollek.task_id", event.task_id.clone().unwrap_or_default()),
        KeyValue::new(
            "pollek.invocation_id",
        ...
        KeyValue::new(
            "pollek.usage.cached_input_tokens",
            event.tokens.cached_input_tokens,
        ),
        KeyValue::new(
            "pollek.usage.reasoning_output_tokens",
            event.tokens.reasoning_output_tokens,
        ),
        KeyValue::new("pollek.cost.total", event.cost.total_cost),
    ];
