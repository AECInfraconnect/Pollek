...
ดูใน dashboard หน้า **Audit & Decision Logs** ได้เช่นกัน นอกจากนี้ยังมีหน้าอื่นๆ ให้ใช้งาน:

- **Auto Discovery** — ดู AI agent ที่ทำงานอยู่ในเครื่อง ตรวจจับโดยการสแกนโปรเซส
- **Shadow AI Inbox** — แจ้งเตือนเมื่อพบกิจกรรม AI ที่ไม่ได้ลงทะเบียน
- **Policy Suggestions** — คำแนะนำ policy ที่สร้างอัตโนมัติจากข้อมูลที่สังเกตได้
- **Cost Ledger** - ใช้ค่า token/cost จริงจาก provider หรือ wrapper ก่อน และติดป้าย estimate เฉพาะกรณีที่เห็นได้แค่ metadata
- **Policy Presets** — ติดตั้ง guardrail สำเร็จรูป (บล็อก Shadow AI, งบประมาณ, ฯลฯ) ในคลิกเดียว
- **Blackbox AI Providers** — จัดการผู้ให้บริการ AI ภายนอกที่ลงทะเบียนไว้
- **Alerts** — การแจ้งเตือนด้านความปลอดภัยและ compliance ทั้งระบบ
...
- **เจอ Error `bootstrap already exists`:** มักเกิดจากการรัน `dek-cli enroll` ซ้ำ หรือมีไฟล์ตั้งค่าเก่าค้างอยู่ วิธีแก้คือ ปิด `dek-core`, ลบโฟลเดอร์ตั้งค่าทิ้ง (`C:\ProgramData\PollekDEK` ใน Windows) แล้วเริ่มทำข้อ 3 ใหม่อีกครั้ง
