...
```text
crates/dek-policy-intent/          # Pollek Policy Intent schema + validators
...
schemas/Pollek-policy-intent.schema.json
schemas/Pollek-bundle-manifest.schema.json
schemas/Pollek-entity.schema.json
schemas/Pollek-capability.schema.json
```
...
entities:
  tenant:
    id: tenant:acme
    trust_domain: spiffe://acme.Pollek.local
  ...
  device:
    ...
    posture: healthy
    spiffe_id: spiffe://acme.Pollek.local/device/win-laptop-001/dek-core
  ...
  workload:
    ...
    spiffe_id: spiffe://acme.Pollek.local/workload/claude-desktop
...
### 7.1 Pollek Cloud / Mock-Cloud with Natural Language AI Editor

```mermaid
sequenceDiagram
  participant Admin
  participant CloudUI as Pollek Cloud UI / Mock-Cloud UI
...
```cedar
permit(
  principal is Pollek::Agent,
  action == Pollek::Action::"mcp.call_tool",
  resource == Pollek::Tool::"filesystem.read_file"
)
when {
...
```rego
package Pollek.policy.pol_allow_openai
...
```json
{
  ...
  "plugins": [
    { "id": "pii-redactor", "abi": "Pollek-plugin-v1", "status": "ready" }
  ],
  "kernel": {
    "linux_ebpf": null,
...
Compatible conceptually with OPA decision logs but extended with Pollek fields:
...
app.post("/v1/policy-intents/draft", (req, res) => {
  ...
  // Deterministic beta stub. Replace with AI orchestrator in Pollek Cloud.
  const ppi = {
    apiVersion: "<your-cloud-domain>/v1alpha1",
    kind: "PolicyIntent",
    metadata: {
...
For Pollek Cloud AI Policy Editor:
...
schemas/
  Pollek-policy-intent.schema.json
  Pollek-bundle-manifest.schema.json
  Pollek-entity.schema.json
  Pollek-capability.schema.json
...
```mermaid
flowchart TB
  subgraph Cloud[Pollek Cloud / Mock-Cloud]
