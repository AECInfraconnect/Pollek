...
- **Production identity** — an OAuth/OIDC bearer (`DEK_CLOUD_API_KEY`) is sent
  when set; SPIFFE/mTLS is applied at the transport layer. Local dev may run
  auth-disabled over loopback.
...
| `DEK_CLOUD_API_KEY` | OAuth/OIDC bearer | *(empty ⇒ omit; dev only)* |
...
Against a real Railway Cloud, set `DEK_CLOUD_URL` to that URL and
`DEK_CLOUD_API_KEY` to a valid token; nothing else changes.
